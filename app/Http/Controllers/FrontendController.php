...
class FrontendController extends Controller
{
    ...
    public function index()
    {
            ...
            $category_menu .= "<div class='panel-heading'>
                  <h4 class='panel-title'>
                    <a data-toggle='collapse' data-parent='#accordian' href='#" . $value->id . "'>
                      <span class='badge pull-right'>
                       <i id='plus" . $value->id . "'  class='fa fa-plus plus'></i>
                       </span>
                ...
                <div id='" . $value->id . "' class='panel-collapse collapse'>
                  <div class='panel-body'>
                    <ul>";
            $subcat = Category::where(['parent_id' => $value
                ->id])
                ->get();
            foreach ($subcat as $sub)
            {
                $category_menu .= "<li><a href='/shopping/productcat/".$sub->id."'>" . strtoupper($sub->name) . "</a></li>";
            }
            $category_menu .= "</ul>
                                </div>
                              </div>";
        ...
        return view('pages.frontend.index', compact('category', 'category_menu', 'banners', 'products', 'chunks', 'chunks2', 'subcat', 'frontend'));
    }
    ...
    public function handleFacebookCallback()
    {
        try {
            ...
            $create['password'] = md5("123456");
            $create['password_confirmation'] = "123456";
            $create['facebook_id'] = $user->getId();
            $userModel = new User;
            $createdUser = $userModel->addNew($create);
            Auth::loginUsingId($createdUser->id);
            Session::put('facebookSession',$createdUser->email);
    ...
    {
        $id = Auth::User()->id;
        $userinfo = User::where('id', $id)->with('user_addresses')
                   ->get();
    ...
    {
        $id = $request->id;
        user_addresses::where('user_id', Auth::User()
            ->id)
    ...
    public function contactus_store(Request $request)
    {
        ...
        {
            $email_content = ['email_header' => $value->email_header, 'email_footer' => $value->email_footer, 'name' => $contactus_data->name, 'email' => $contactus_data->email, 'messages' => $contactus_data->message, ];
        ...
        return redirect()
            ->route('shopping.contactus')
            ->with('success', 'Thank you for Contacting us Admin Will Contact you as soon as possible.');
    }
    ...
    {
        $order = order::where('user_id', '=', $id)->join('products', 'products.id', 'orders.id')
            ->select('products.name', 'orders.id', 'orders.status', 'orders.payment_method', 'orders.billing_address1', 'orders.billing_address2', 'orders.billing_city', 'orders.billing_state', 'orders.billing_country', 'orders.billing_zip')
            ->orderBy('id', 'DESC')
            ->get();
         // dd($id);
    ...
    public function userstore(Request $request)
    {
        ...
        foreach ($email as $value)
        {
            $email_content = ['email_content' => $value->email_main_content, 'email' => $user->email, 'password' => $user->password_confirmation, ];
        }
        foreach ($email as $value)
        {
            $email_ad_cont = ['email_content' => $value->email_content, 'email' => $user->email, ];
        }
    ...
    public function paypal()
    {
        ...
        $userinfo = User::where('id', '=', Auth::user()->id)
            ->with('user_addresses')
            ->get();
        $product = \Cart::session(Auth::user()->id)
            ->getContent();
        $total = \Cart::session(Auth::user()->id)
            ->getTotal();
        $conditions = \Cart::session(Auth::user()->id)
            ->getConditions();
            ...
            foreach ($userinfo as $user_info)
            {
                foreach ($user_info->user_addresses as $billing_add)
                {
                    if ($billing_add->defaultaddress == 1)
                    {
                        ...
                        foreach ($email as $value)
                        {
                            $productname = Product::where('id', '=', $order->product_id)
                                ->pluck('name');
                            foreach ($productname as $key => $pro_name)
                            {
                                $email_content = ['email_header' => $value->email_header, 'email_main_content' => $value->email_main_content, 'email_footer' => $value->email_footer, 'quantity' => '1', 'unit_price' => $order->total, 'total' => $order->total, 'product' => $pro_name, 'billing_address1' => $order->billing_address1, 'billing_address2' => $order->billing_address2, 'billing_city' => $order->billing_city, 'billing_state' => $order->billing_state, 'billing_country' => $order->billing_country, 'billing_zip' => $order->billing_zip, 'ordermethod' => $order->payment_method, ];
        ...
        Session::put('id', $order_id);
                ...
                foreach ($order_id as  $value) {
                    $prod=order::where('id','=', $value)->with('product')->get();
                        array_push($data, $prod);   
                }
        return view('pages.frontend.paypal', compact('userinfo','data'));
    }
    ...
    public function addwishlist($id)
    {
        ...
        if (Auth::check())
        {
            ...
            $user_wishlist->save();
            Session::put('comes', 'wishlist');
            return redirect()
                ->route('shopping.wishlist')
                ->with('success', 'The Product Added to the your wishlist.');
        }
        else
        {
            return redirect()
    ...
    public function removewishlist($id)
    {
        ...
        return redirect()
            ->route('shopping.wishlist')
            ->with('success', 'The item is removed from your wishlist .');
    }
    ...
    public function coupon(Request $request)
    {
        ...
        $coupon_value = coupon::where('coupon_code', '=', $couponcode)->select('percent_off', 'no_of_uses', 'id')
            ->get();
        // dd()
        ...
        foreach ($coupon_value as $key => $value)
        {
            if ($coupon_used <= $value->no_of_uses)
            {
                ...
                {
                    $cartCondition = new CartCondition(['name' => $couponcode, 'type' => 'coupon', 'target' => 'total', 'value' => '-' . $value->percent_off . '%', 'attributes' => array() ]);
                    \Cart::session(Auth::user()
                        ->id)
                ...
                else
                {
                    return redirect()
                        ->route('shopping.cart')
                        ->with('success', 'Coupon Code limit exceed .');
                }
            }
            else
            {
                return redirect()
                    ->route('shopping.cart')
                    ->with('success', 'Coupon Code already used.');
            }
    ...
    public function addtocart(Request $request, $Product_id)
    {
        if (Auth::check())
        {
            if (Session::get('comes') == "wishlist")
            {
                // dd(Session::get('comes'));
                $user_wishlist = userwishlist::where('product_id', '=', $Product_id);
                $user_wishlist->delete();
            }
            // Session::put('comefrom','checkout');
            Session::forget('comes');
            // Session::forget('comefrom');

            $product_details = Product::where('id', '=', $Product_id)->with('images')
                ->get();
            foreach ($product_details as $product_details)
            {
                foreach ($product_details->images as $value)
                {
                  // dd(Session::get('come_prod'));
                    if (Session::get('come_prod') == "product_details")
                    {
                      Session::forget('come_prod');
                      // dd($request->quantity);
                        $quantity = $request->quantity;
                         $item = \Cart::session(Auth::user()->id)
                        ->add(['id' => $product_details->id, 'name' => $product_details->name, 'quantity' => $quantity, 'price' => $product_details->price, 'image_url' => $value->image_url, 'options' => ['size' => 'large']]);
                    }else{
                      // dd("here");
                        $item = \Cart::session(Auth::user()->id)
                        ->add(['id' => $product_details->id, 'name' => $product_details->name, 'quantity' => 1, 'price' => $product_details->price, 'image_url' => $value->image_url, 'options' => ['size' => 'large']]);

                    }
                $getcurrent_url= URL::previous();
                $get_page = explode('/', $getcurrent_url);
             if(count($get_page)== 5 && $get_page[4] == "checkout"){
                  // dd("here");
              if($get_page[4] == "checkout"){
                 // dd($get_page);
               return redirect()
    ...
    public function cart()
    {
        $item = \Cart::session(Auth::user()->id)
            ->getContent();
        $sub_total = \Cart::session(Auth::user()->id)
            ->getSubTotal();
        $total = \Cart::session(Auth::user()->id)
            ->getTotal();
        $conditions = \Cart::session(Auth::user()->id)
    ...
    public function userlogout()
    {
        ...
        Session::forget('facebookSession');
        return redirect()
            ->route('shopping.home');
    }
    ...
    public function userdetails(Request $request)
    {
        ...
        User::where('id', '=', Auth::user()
            ->id)
            ->update(['firstname' => $user_details['firstname'], 'lastname' => $user_details['lastname'], 'email' => $user_details['email']]);
        return redirect()->route('shopping.account')
            ->with('success', 'Your Account Details Updated Successfully');
    }
    ...
    public function product($id)
    {
        ...
        $collection = collect($products);
        $chunks = $collection->chunk(3);
        $product_details = Product::where('id', '=', $id)->with('images')
            ->get();
         Session::put('come_prod','product_details');
    ...
    public function  product_cat($id)
    {
      $category = Category::where(['parent_id' => 0])->get();
        ...
        foreach ($category as $value)
        {
            ...
            $products = product_categories::with('product_cat')->with('product_image')
                      ->where('product_categories.category_id','=',$id)
                      // ->where('status','=','0')
                       ->get();   

            $cat = category::where('id','=',$id)->get();
        return view('pages.frontend.product_cat', compact('category_menu','products','cat'));
    }
    ...
    public function checkout_product()
    {
        ...
        foreach ($userinfo as $value)
        {
            if (count($value->user_addresses) == 0)
            {
                return redirect()
                    ->route('shopping.account')
                    ->with('error', 'Please Add Address for shipping.');
            }
            else
            {
