          ...
          <tr>
           ...
           <tr>
            ...
            <td><b>Order Status  :</b></td>
             <td> 
                <form method="POST" action="{{route('order.orderstatus', $userdet->id)}}">
                @csrf 
                <div>
                 <select name="order_status">
                   <option value="">select</option>
                   <option value="1" {{($userdet->order_status == 1)? 'selected':''}}>on its way</option>
                   <option value="2" {{($userdet->order_status == 2)? 'selected':''}}>Out for delivery</option>
                   <option value="3" {{($userdet->order_status == 3)? 'selected':''}}>Delivered</option>
                   <option value="4" {{($userdet->order_status == 4)? 'selected':''}}>Attempted delivery</option>
                 </select>
                </div>
                 <button style="margin-top:5px;" type="submit" class="btn btn-sm btn-info">update order Status</button>
               </form>
